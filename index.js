module.exports = {
	etelecomZaloOaTrigger: {
		// Regular node
		'dist/nodes/EtelecomZaloOaTrigger/EtelecomZaloOaTrigger.node.js':
			'dist/nodes/EtelecomZaloOaTrigger/EtelecomZaloOaTrigger.node.js',
		// Static data needed by the node
		'dist/nodes/EtelecomZaloOaTrigger/EtelecomZaloOaTrigger.node.json':
			'dist/nodes/EtelecomZaloOaTrigger/EtelecomZaloOaTrigger.node.json',
		// Icons
		'dist/nodes/EtelecomZaloOaTrigger/etelecom.svg':
			'dist/nodes/EtelecomZaloOaTrigger/etelecom.svg',
	},

	etelecomZaloOaSendMessage: {
		// Regular node
		'dist/nodes/EtelecomZaloOaSendMessage/EtelecomZaloOaSendMessage.node.js':
			'dist/nodes/EtelecomZaloOaSendMessage/EtelecomZaloOaSendMessage.node.js',
		// Static data needed by the node
		'dist/nodes/EtelecomZaloOaSendMessage/EtelecomZaloOaSendMessage.node.json':
			'dist/nodes/EtelecomZaloOaSendMessage/EtelecomZaloOaSendMessage.node.json',
		// Icons
		'dist/nodes/EtelecomZaloOaSendMessage/etelecom.svg':
			'dist/nodes/EtelecomZaloOaSendMessage/etelecom.svg',
	},

	etelecomZaloOaSendZns: {
		'dist/nodes/EtelecomZaloOaSendZns/EtelecomZaloOaSendZns.node.js':
			'dist/nodes/EtelecomZaloOaSendZns/EtelecomZaloOaSendZns.node.js',
		'dist/nodes/EtelecomZaloOaSendZns/EtelecomZaloOASendZns.node.json':
			'dist/nodes/EtelecomZaloOaSendZns/EtelecomZaloOASendZns.node.json',
		'dist/nodes/EtelecomZaloOaSendZns/etelecom.svg':
			'dist/nodes/EtelecomZaloOaSendZns/etelecom.svg',
	},

	etelecomZaloOaRequestUserInfo: {
		'dist/nodes/EtelecomZaloOaRequestUserInfo/EtelecomZaloOaRequestUserInfo.node.js':
			'dist/nodes/EtelecomZaloOaRequestUserInfo/EtelecomZaloOaRequestUserInfo.node.js',
		'dist/nodes/EtelecomZaloOaRequestUserInfo/EtelecomZaloOaRequestUserInfo.node.json':
			'dist/nodes/EtelecomZaloOaRequestUserInfo/EtelecomZaloOaRequestUserInfo.node.json',
		'dist/nodes/EtelecomZaloOaRequestUserInfo/etelecom.svg':
			'dist/nodes/EtelecomZaloOaRequestUserInfo/etelecom.svg',
	},

	etelecomZaloOaRequestConsent: {
		'dist/nodes/EtelecomZaloOaRequestConsent/EtelecomZaloOaRequestConsent.node.js':
			'dist/nodes/EtelecomZaloOaRequestConsent/EtelecomZaloOaRequestConsent.node.js',
		'dist/nodes/EtelecomZaloOaRequestConsent/EtelecomZaloOaRequestConsent.node.json':
			'dist/nodes/EtelecomZaloOaRequestConsent/EtelecomZaloOaRequestConsent.node.json',
		'dist/nodes/EtelecomZaloOaRequestConsent/etelecom.svg':
			'dist/nodes/EtelecomZaloOaRequestConsent/etelecom.svg',
	},
};
